...
u32 ath_calcrxfilter(struct ath_softc *sc)
{
	...
	rfilt = ATH9K_RX_FILTER_UCAST | ATH9K_RX_FILTER_BCAST
	  | ATH9K_RX_FILTER_MCAST |ATH9K_RX_FILTER_PHYERR   ;//JIGS
	...
	if (sc->rx.rxfilter & FIF_CONTROL) //for control packets
			    ...
			    bool *decrypt_error)
{
	...
	if (rx_stats->rs_status != 0) {
		...
		status_mask = ATH9K_RXERR_DECRYPT | ATH9K_RXERR_MIC |
			      ATH9K_RXERR_KEYMISS | ATH9K_RXERR_PHY ; //JIGS
...
static void ath9k_radiotap_add(struct ath_softc *sc,
			       struct sk_buff *skb,
			       struct ath_rx_status *rs,
			       int rt_len)
{
  ...
  if(skb->len> 4){
    ...
    const u16 caplen = rs->rs_datalen ;
    ...
    rt->jig.antenna_ =  status->antenna;
    rt->jig.caplen_  = caplen;
    rt->jig.snaplen_ =  snaplen ;
        
  status->flag |= RX_FLAG_EXTRA_RADIOTAP;
/*	
	
  static int asl1=0;
  if(asl1<5){
    printk("abhinav driver :ofdmerr: %u | %u \n",(u32) rt->hw_cntr_ofdmerr , ah->stats.ast_ani_ofdmerrs  );
    printk("abhinav driver :cckerr: %u | %u \n", (u32) rt->hw_cntr_cckerr  , ah->stats.ast_ani_cckerrs  );
    printk("abhinav driver : rssi: %u | %u \n" , rt->rssi, rs->rs_rssi );
    rt->isset =asl1;
    printk("abhinav driver : rt->footer_version %u \n", rt->footer_version);
    asl1++;
  }
  */
	}
		     ...
		     struct ath_rx_status *rs)
{
  ...
  int rt_len = sizeof(struct ath9k_radiotap)  ; //ath9k_radiotap_len(sc, rs);
    ...
    static int asl=0;
    if(asl<5){
  //    printk("abhinav: start headroom=%d skb=%p skb->head=%p skb->data=%p skb->tail=%p skb->end=%p \n",skb_headroom(skb),skb,skb->head,skb->data,skb->tail,skb->end);
    }
		
    if (skb_headroom(skb) < rt_len  &&  pskb_expand_head(skb, rt_len  , 0, GFP_ATOMIC)) {
      static int askb=0;
      if(askb<3)
        printk("abhinav: ERROR mem %d\n",askb);
...
#endif
{
	...
	static int av=0;
	if(av<2)
	  printk("abhinav: in ath rx tasklet %d \n",av++);
#endif
	do {
