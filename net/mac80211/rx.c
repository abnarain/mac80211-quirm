...
static struct sk_buff *remove_monitor_info(struct ieee80211_local *local,
					   struct sk_buff *skb)
#else
static struct sk_buff *remove_monitor_info_jigs(struct ieee80211_local *local,
						struct ieee80211_rx_status *status,
						struct sk_buff *skb,
						struct custom_hdr_t * custum_hdr)
#endif 
{
#ifdef JIGS
...
#ifdef JIGS
	static int aj=0;
	struct ath9k_radiotap *a9k = (struct ath9k_radiotap *)skb->data;

	if (status->band == IEEE80211_BAND_5GHZ){
	  a9k->jig.channel_ =  IEEE80211_CHAN_OFDM | IEEE80211_CHAN_5GHZ;
	}
	else if (status->flag & RX_FLAG_HT){
	  a9k->jig.channel_ =  IEEE80211_CHAN_DYN | IEEE80211_CHAN_2GHZ ;
	}else if (rate->flags & IEEE80211_RATE_ERP_G){
	  a9k->jig.channel_ = IEEE80211_CHAN_OFDM | IEEE80211_CHAN_2GHZ ;
	}
	else if (rate)
	  a9k->jig.channel_ = IEEE80211_CHAN_CCK | IEEE80211_CHAN_2GHZ;
	else
	  a9k->jig.channel_ = IEEE80211_CHAN_2GHZ ;

	// IEEE80211_RADIOTAP_RATE
	if (!rate || status->flag & RX_FLAG_HT) {
	  //      Without rate information don't add it. If we have MCS information is a separate field in radiotap,
	  //      added below. The byte here is needed as padding for the channel though, so initialise it to 0.
	  a9k->jig.rate_idx_ = status->rate_idx ;
	} else {
...
#ifdef JIGS
  struct custom_hdr_t custum_hdr ;
 	memset(&custum_hdr,0,sizeof(struct custom_hdr_t));
#endif
...
#else
	needed_headroom = sizeof(struct jigdump_hdr);
#endif
...
/*		if(status != NULL) crashes the kernel in 210 seconds  
		...
		*/	
		...
		if (skb_headroom(skb) < needed_headroom &&
			...
			static int kv=0;
			if(kv <5)
			printk("abhinav: cant get headroom %d \n",kv++);			
			dev_kfree_skb(skb);
			return NULL;
		}
		*/
...
#else
	ieee80211_add_rx_radiotap_header_jigs(local, skb, rate, needed_headroom, &custum_hdr);	
#endif

	skb_pull(skb,sizeof(struct ieee80211_radiotap_header ));
